...
namespace GestionEspav
{
    internal class NotesClass
    {
        ...
        public bool InsertNote(float note, int idmatiere, int idEtudiant, string typenote)
        {
            MySqlCommand command = new MySqlCommand("INSERT INTO `notes`(`note`, `idmatiere`, `idEtudiant`, `TypeNote`) VALUES(@no, @im, @ie , @tn)", connect.getconnection);
            ...
            command.Parameters.Add("@tn", MySqlDbType.VarChar).Value = typenote;
            connect.openConnect();
